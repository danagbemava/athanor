...
- `cd apps/worker && go run ./cmd/worker healthcheck`

Preferred Docker paths:

- Built-image full stack: `make compose-up`
- Source-mounted Docker dev stack: `make compose-up-dev`
...
- The worker exposes a lightweight CLI `healthcheck` command used by Docker health checks to verify Redis connectivity.
